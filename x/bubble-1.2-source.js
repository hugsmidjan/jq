 ...
 *    - For now, a call to event.preventDefault will cause nothing.
 * TO-DO:
 *    - Trigger native bindings ( f.e: onclick, onmouseover,etc )
 *    - Maybe wrap the "while" part with a setTimeout, to make it asynchronous.
 **/

(function( $ ){

  $.fn.bubble = function(type, data){
    data = $.makeArray(data || []);//ensure an array of data
    var parts = type.split('.');
    type = parts[0];
    return this.each(function(){
      var node = this, event = new Event( type, node ), args = [ event ].concat( data );
      var evs, ev, handler;
      setTimeout(function(){//this will give each node it's own execution time
        while( node != document && !event.cancelBubble ){//keep going up till you hit the document or propagation is stopped.
          if( evs = $.data(node,'events') && $.data(node,'events')[type] ){//does this node have events attached?
