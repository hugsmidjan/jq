    ...
    av : {
      ...
      addInlineLabels : function ( ctrllist ) {
        ...
        $.each(ctrllist, function (i) {
          ...
          ctrl.attr( 'lang', lang );
          // build error
          var errorMessage = ctrl.data('av-errortype') === 'type'
              ? ctrl.data('av-error') || $.av.getText( 'inlineTypeMsg', lang )
              : $.av.getText( 'inlineReqMsg', lang );
      ...
      alertErrors : function ( ctrllist, contexts ) {
        ...
        $.each(ctrllist, function () {
          ...
          if ( ctrl.data('av-errortype') === 'type' ) {  // is it a type error?
  ...
  $.fn.extend({
    ...
    isValid : function ( report ) {
      ...
      this.each(function () {
        ...
        if ( report ) {
          var errMsgWrap = context.is(':input') ? context.closest('[class^="fi_"], [class*=" fi_"]') : context;
          errMsgSel = errMsgSel || '.' + conf.inlineErrorClass + ', a.' + conf.nextErrorLinkClass;
